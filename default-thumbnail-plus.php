...
class DefaultPostThumbnailPlugin {
                ...
                foreach($dpt_options as $key => $dpt_option_arr) {
                    ...
                    foreach($dpt_option_arr as $dpt_option) {
                        if( is_object_in_term($post_id, $key, $dpt_option['value']) ) {
                            $default_post_thumbnail = $dpt_option['attachment_id'];
                        } 
                    }
                }
            }
	...
	static function get_attr_string($size) {
    ...
    static function handle_options_update() {
		...
		update_option( 'dpt_use_embedded_img', $_POST['dpt_use_embedded_img'] );
		update_option( 'dpt_use_embedded_video', $_POST['dpt_use_embedded_video'] );
